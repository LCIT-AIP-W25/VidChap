...
# Function to parse transcript and extract entries
...
# Function to split transcript entries into chunks
...
# Function to save chunks into files
def save_chunks_to_files(chunks):
    ...
    for idx, chunk in enumerate(chunks):
        ...
        with open(f'chunks/chunk_{idx + 1}.txt', 'w') as f:
            f.write(chunk_text)

# Function to process chunks via the GROQ API
@dask.delayed
            ...
            json={
                "messages": [{
                    ...
                    "content": f"""Analyze this video transcript segment and generate chapter names with timestamps. \
                                Make sure the timings match with the appropriate boundaries according to the given transcript format: \
...
# Function to parse chapter response from API
...
# Main function to process the transcript on the fly
def main():
    client = Client()
    ...
    # Parse and split the transcript into chunks
    ...
    # Process chunks with the GROQ API
