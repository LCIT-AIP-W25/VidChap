...
def parse_transcript(transcript_text):
    ...
    # Debugging: log the first few timestamps to check their range
    logging.debug(f"Parsed timestamps: {entries[:5]}")
    return entries
...
def process_chunk(chunk_entries):
    try:
        ...
        # Dynamically calculate timeout based on chunk size (more words = more processing time)
        timeout = min(30, max(15, len(chunk_text) // 1000))
        
        logging.info(f"Processing chunk of {len(chunk_entries)} entries, estimated timeout: {timeout}s")
        ...
        if response.status_code != 200:
            ...
            if response.status_code == 429:  # Rate limit exceeded
                ...
                logging.info(f"Rate limit exceeded. Waiting for {wait_time}s...")
...
def main():
    ...
    for chap in all_chapters:
        logging.info(f"{chap['start']:.2f}s - {chap['end']:.2f}s: {chap['name']}")
