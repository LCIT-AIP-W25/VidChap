...
def download_youtube_audio_in_memory(url: str) -> Dict:
    """
    Download YouTube audio to memory instead of saving to disk.
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                # Prepare filename based on the extracted info
                ...
                mono_audio.export(wav_buffer, format="wav")
                wav_buffer.seek(0)
            ...
            logger.error(f"In-memory download failed: {str(e)}")
            raise HTTPException(status_code=500, detail=f"Failed to download YouTube audio in memory: {str(e)}")



def split_audio_in_memory(audio_data: np.ndarray, sample_rate: int, chunk_length_ms: int = 30000) -> List[Dict]:
    """
    Split in-memory audio data into chunks for processing.
    ...
    """
    try:
        # Convert chunk length to samples
        chunk_length_samples = int(sample_rate * chunk_length_ms / 1000)
        ...
        for start in range(0, len(audio_data), chunk_length_samples):
            ...
            chunk = chunk.astype(np.float32) / np.max(np.abs(chunk))
        ...
        logger.error(f"In-memory audio splitting failed: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to split audio in memory: {str(e)}")

def process_youtube_video_in_memory(url: str) -> Tuple[str, str]:
    """
    Main processing pipeline for YouTube URLs using in-memory processing.
    """
    try:
        # Download audio to memory
        audio_info = download_youtube_audio_in_memory(url)
        
        # Split audio in memory
        chunks = split_audio_in_memory(
            audio_info['audio_data'], 
            audio_info['sample_rate']
        )
        
        # Rest of your existing transcription and chapter extraction logic remains the same
        # You'll pass the in-memory chunks instead of an audio file path
        # (Use the existing transcribe_chunk and other functions)
        full_text, segments = transcribe_with_timestamps_in_memory(
            chunks, 
            language="en"
        )
        ...
        logger.error(f"In-memory processing failed: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Processing failed: {str(e)}")

def transcribe_with_timestamps_in_memory(
    chunks: List[Dict],
    ...
    """
    Transcribe in-memory audio chunks using multiprocessing.
    """
    try:
        ...
        logger.error(f"In-memory transcription failed: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to transcribe audio in memory: {str(e)}")
...
async def process_youtube(request: YouTubeURLRequest):
    ...
    try:
        full_transcript, chapters = process_youtube_video_in_memory(request.url)
